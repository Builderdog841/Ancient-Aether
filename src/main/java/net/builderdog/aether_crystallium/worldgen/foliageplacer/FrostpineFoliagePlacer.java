...
public class FrostpineFoliagePlacer extends FoliagePlacer {
    public static final Codec<FrostpineFoliagePlacer> CODEC = RecordCodecBuilder.create((codec) -> foliagePlacerParts(codec)
            .and(IntProvider.codec(0, 24).fieldOf("trunk_height").forGetter((placer) -> placer.trunkHeight))
            .apply(codec, FrostpineFoliagePlacer::new));
    private final IntProvider trunkHeight;

    public FrostpineFoliagePlacer(IntProvider radius, IntProvider offset, IntProvider height) {
        super(radius, offset);
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.FROSTPINE_FOLIAGE_PLACER.get();
    }
