...
public class AncientPlayerAttachment implements INBTSynchable {
    ...
    public AncientPlayerAttachment() {
    }
    ...
    private void handleRemoveDarts(Player player) {
        if (!player.level().isClientSide()) {
            if (getAeronauticDartCount() > 0) {
                ...
                if (removeAeronauticDartTime <= 0) {
                    setSynched(player.getId(), Direction.CLIENT, "setAeronauticDartCount", getAeronauticDartCount() - 1);
                }
            }
        }
    }
    ...
    private void handleWingRotation(Player player) {
        if (player.level().isClientSide()) {
            ...
            if (AncientEquipmentUtil.hasFullValkyrumSet(player)) {
