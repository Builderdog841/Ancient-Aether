...
public class AncientBoatRenderer<T extends AncientBoat> extends EntityRenderer<T> {
    private final Map<AncientBoat.Type, Pair<ResourceLocation, ListModel<Boat>>> boatResources;

    public AncientBoatRenderer(EntityRendererProvider.Context renderer, boolean hasChest) {
        super(renderer);
        shadowRadius = 0.8f;
        boatResources = Stream.of(AncientBoat.Type.values()).collect(ImmutableMap.toImmutableMap(type -> type, type ->
                Pair.of(type.getTexture(hasChest), createBoatModel(renderer, type, hasChest))));
    }

    private ListModel<Boat> createBoatModel(EntityRendererProvider.Context renderer, AncientBoat.Type type, boolean hasChest) {
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(AncientBoat boat) {
