...
public class WyndZephyr extends FlyingMob implements Enemy {
    public static final EntityDataAccessor<Integer> DATA_ATTACK_CHARGE_ID = SynchedEntityData.defineId(WyndZephyr.class, EntityDataSerializers.INT);
    public int scale;
    public int scaleAdd;
    public float tailRot;
    public float tailRotAdd;

    public WyndZephyr(EntityType<? extends WyndZephyr> type, Level level) {
        super(type, level);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(5, new SnowballAttackGoal(this));
    ...
    public static boolean checkWyndZephyrSpawnRules(EntityType<? extends WyndZephyr> wyndZephyr, LevelAccessor level, MobSpawnType reason, BlockPos pos, RandomSource random) {
        return level.getDifficulty() != Difficulty.PEACEFUL && Mob.checkMobSpawnRules(wyndZephyr, level, reason, pos, random) && (reason != MobSpawnType.NATURAL || random.nextInt(11) == 0) && level.canSeeSky(pos);
    }

    @Override
    public void aiStep() {
        super.aiStep();
        if (this.getY() < level().getMinBuildHeight() - 2 || this.getY() > level().getMaxBuildHeight()) {
            this.discard();
        }
    ...
    protected static class SnowballAttackGoal extends Goal {
        private final WyndZephyr parentEntity;
        public int attackTimer;

        public SnowballAttackGoal(WyndZephyr veloxzephyr) {
            this.parentEntity = veloxzephyr;
        }
        ...
        public void tick() {
            ...
            if (target.distanceToSqr(this.parentEntity) < 40 * 40 && this.parentEntity.hasLineOfSight(target)) {
                ...
                if (this.attackTimer == 10) {
                    this.parentEntity.playSound(Objects.requireNonNull(this.parentEntity.getAmbientSound()), 3.0F, (level.random.nextFloat() - level.random.nextFloat()) * 0.2F + 1.0F);
                    ...
                    WyndZephyrSnowball snowball = new WyndZephyrSnowball(level, this.parentEntity, accelX, accelY, accelZ);
    ...
    protected static class RandomFlyGoal extends Goal {
        ...
        public RandomFlyGoal(WyndZephyr entity) {
        ...
        public void start() {
            ...
            double d0 = parentEntity.getX() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double d1 = parentEntity.getY() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double d2 = parentEntity.getZ() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            parentEntity.getMoveControl().setWantedPosition(d0, d1, d2, 1.0);
        }
    }
    ...
    protected static class MoveHelperController extends MoveControl {
        ...
        public MoveHelperController(WyndZephyr veloxzephyr) {
            super(veloxzephyr);
            this.parentEntity = veloxzephyr;
        }
    ...
    protected static class LookAroundGoal extends Goal {
        ...
        public LookAroundGoal(WyndZephyr wyndZephyr) {
            this.parentEntity = wyndZephyr;
