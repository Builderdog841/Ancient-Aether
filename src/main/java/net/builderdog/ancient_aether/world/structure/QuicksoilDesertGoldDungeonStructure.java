...
public class QuicksoilDesertGoldDungeonStructure extends Structure {
    public static final Codec<QuicksoilDesertGoldDungeonStructure> CODEC = RecordCodecBuilder.create(builder -> builder.group(
            settingsCodec(builder),
            Codec.INT.fieldOf("stubcount").forGetter(o -> o.stubIslandCount)
    ).apply(builder, QuicksoilDesertGoldDungeonStructure::new));

    private final int stubIslandCount;

    public QuicksoilDesertGoldDungeonStructure(StructureSettings settings, int stubIslandCount) {
        super(settings);
                ...
                if (featureType < treeWeight + flowerWeight) {
                    ...
                    if (iterateColumn(level, mutable, minY, maxY)) {
                        if (featureType < treeWeight) {
                            PlacedFeature tree = PlacementUtils.inlinePlaced(level.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolderOrThrow(AncientAetherConfiguredFeatures.GOLDEN_LARGE_CACTUS)).get();
    ...
    public StructureType<?> type() {
        return AncientAetherStructureTypes.QUICKSOIL_DESERT_GOLD_DUNGEON.get();
    }
}