...
public class HookedTrunkPlacer extends TrunkPlacer  {
    ...
    protected @NotNull TrunkPlacerType<?> type() {
        return AncientAetherTrunkPlacers.HOOKED_TRUNK_PLACER.get();
    }

    @Override
    public @NotNull List<FoliagePlacer.FoliageAttachment> placeTrunk(@NotNull LevelSimulatedReader level, @NotNull BiConsumer<BlockPos, BlockState> blockSetter, @NotNull RandomSource random, int height, BlockPos pos, @NotNull TreeConfiguration config) {
    ...
    protected void placeTrunk(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter, RandomSource random, BlockPos pos, BlockStateProvider trunkProvider) {
        this.placeTrunk(level, blockSetter, random, pos, Function.identity(), trunkProvider);
    }

    protected void placeTrunk(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter, RandomSource random, BlockPos pos, Function<BlockState, BlockState> propertySetter, BlockStateProvider trunkProvider) {
        if (this.validTreePos(level, pos)) {
            blockSetter.accept(pos, propertySetter.apply(trunkProvider.getState(random, pos)));
        }
    }

    protected void placeBranch(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter, RandomSource random, BlockPos pos, TreeConfiguration config) {
        this.placeBranch(level, blockSetter, random, pos, config, Function.identity());
    }

    protected void placeBranch(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter, RandomSource random, BlockPos pos, TreeConfiguration config, Function<BlockState, BlockState> propertySetter) {
